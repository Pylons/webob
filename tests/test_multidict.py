...
class BaseDictTests:
    ...
    def _get_instance(self, **kwargs):
        data = multidict.MultiDict(kwargs) if kwargs else self.data.copy()
    ...
    def test_dict_of_lists(self):
        assert self.d.dict_of_lists() == {"a": [text_("é"), "e", "f"], "b": ["1"]}
...
class NestedMultiDictTestCase(BaseDictTests, unittest.TestCase):
    ...
    def test_update(self):
        ...
        self.assertRaises(KeyError, d.update, {"x": 1})
    ...
    def test_nonzero(self):
        d = self._get_instance()
        assert d.__nonzero__() is True
        d.dicts = [{}]
        assert d.__nonzero__() is False
        assert not d
