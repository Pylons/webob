...
def tween(handler):
    """Tween to set Host header according to RFC 7239 Forwarded header.

    You can use this tween if you have an application that sits behind
    a trusted proxy that you know sets the RFC 7239 Forwarded header. The
    tween adjusts the Host header according to the information in this
    header.
    ...
    **Warning**: Use this tween *only* when you know your application
    lives behind a trusted proxy that sets the Forwarded header. The
    Forwarded header can be used by attackers to affect the URLs
    generated by your application and make them point to another host
    altogether.
    """
    def tween(request):
